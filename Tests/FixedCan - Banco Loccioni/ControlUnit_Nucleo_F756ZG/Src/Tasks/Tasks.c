...
void TaskInit(void)
{
	...
	HAL_ADC_Start_DMA(&hadc1, (uint16_t*)adc_buf, ADC_BUFFER_LENGTH);
...
void TaskFast(void)
{
	...
	// TODO: De-comment this code to display serial logs
...
/* ***********************************************************************
 * CYCLIC SETPOINTS TRANSMISSION (every 5ms)
 * 
 * This function is called by the scheduler every 5ms to ensure that
 * Setpoints1 messages are sent to the inverter with deterministic timing.
 * The inverter's watchdog requires messages at least every 50ms, so a 5ms
 * period provides a safety margin.
 * 
 * This is critical for fixed CAN configuration: the inverter activates its
 * watchdog on the FIRST Setpoints1 it receives, and any gap > 50ms causes
 * error 3587 (Failure of setpoint message).
 * ***********************************************************************/
void TaskSetpointsCyclic(void)
{
    // Get current time from the scheduler's tick counter
    // Each tick is 1ms, so we can use it directly
    
    // Send Setpoints1 for inverter 1 (cyclic transmission every 5ms)
	APP_Update(adc_buf[0]);
    Inverter_SendSetpointsCyclic(&inverter_104, tickCounter);
    
    // Send Setpoints1 for inverter 2 if it's used
    // Uncomment when inverter_204 is enabled:
    // Inverter_SendSetpointsCyclic(&inverter_204, tickCounter);
}
 ...
 * Motor Functions
 * ***********************************************************************/
void HandleInverterCommunication(void)
{
    // Questa funzione viene chiamata ogni ~1 ms (SysTick)
    static uint16_t setpointCounterMs = 0U;

    uint8_t activationValue = 1; // per ora sempre ON; in futuro leggerai un ingresso reale

    setpointCounterMs++;

    // --------------------------------------------------------------------
    // RPM ramp logic:
    // - If controller is active, ramp up to 18000 RPM
    // - Otherwise, force RPMs to 0
    // --------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------
    // Update setpoints only when controller is active
    // (as per AMK CAN-Fixed flowchart)
    // --------------------------------------------------------------------
    if (inverter_104.state == CONTROLLER_ACTIVE) {
        inverter_104.setpoints1 = (Setpoints1){
            cbDcOn | cbEnable | cbInverterOn,
            (int16_t)RPMs,
            (int16_t)TORQUE_POS,
            (int16_t)TORQUE_NEG
        };
...
//        inverter_204.setpoints1 = (Setpoints1){
//            cbDcOn | cbEnable | cbInverterOn,
//            (int16_t)RPMs,
//            (int16_t)TORQUE_POS,
//            (int16_t)TORQUE_NEG
...
//    }
    ...
    // --------------------------------------------------------------------
    // Invia i setpoint agli inverter ogni ~10 ms
    // (margine ampio rispetto ai 50 ms del telegram failure AMK)
    // --------------------------------------------------------------------
    if (setpointCounterMs >= 10U) {
        setpointCounterMs = 0U;

        if (activationValue) {
            Inverter_Activate(&inverter_104);
            //Inverter_Activate(&inverter_204);
        } else {
            Inverter_Deactivate(&inverter_104);
            //Inverter_Deactivate(&inverter_204);
        }
    }

    // --------------------------------------------------------------------
    // Telemetria verso il volante (temperatura + APPs_flt)
    // COMMENTATA per ridurre il traffico CAN durante il debug
    // --------------------------------------------------------------------
    /*
    {
        ...
        uint8_t rpmData[2];  // Array to hold the APPs_flt data (2 bytes)

        rpmData[0] = (uint8_t)(APPs_flt & 0xFF);         // LSB
        rpmData[1] = (uint8_t)((APPs_flt >> 8) & 0xFF);  // MSB

        Transmit_CAN_Message(&hcan1, 0x031, 2, rpmData);
    }
    */
}
