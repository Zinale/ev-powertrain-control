...
// Cyclic transmission function (called by scheduler at fixed intervals, e.g., every 5ms)
void Inverter_SendSetpointsCyclic(Inverter* inverter, uint32_t currentTimeMs);

// Transmission counter management (called by scheduler before cyclic tasks)
void Inverter_IncrementTransmissionCounter(void);
