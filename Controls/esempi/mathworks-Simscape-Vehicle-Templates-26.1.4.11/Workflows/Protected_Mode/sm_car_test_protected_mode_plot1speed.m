...
plot(logsout_xCar.Data, logsout_yCar.Data, 'LineWidth', 1)
axis equal
grid on
title('Position')
ylabel('Position (m)')
...
subplot(2,2,1)
text(0.05,0.2,sprintf('xInitial: %0.2f m\nyInitial: %0.2f m',...
    logsout_px0,logsout_py0),'Units','Normalized','Color',[1 1 1]*0.5);
%if(isfield(Vehicle,'config'))
%    config_string = Vehicle.config;
%else
    config_string = get_param([bdroot '/Vehicle_sys'],'ModelFile');
%end
