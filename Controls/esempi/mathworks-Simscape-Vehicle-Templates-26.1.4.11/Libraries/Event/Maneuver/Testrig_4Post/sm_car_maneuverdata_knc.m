...
for i=1:length(Instance_List)
    ...
    mdata.(Instance).tRange.LatFrcPara_All  = [LA_timeline(1) LA_timeline(5)];
    mdata.(Instance).tRange.LatFrcIn        = [LA_timeline(2) LA_timeline(3)];
    mdata.(Instance).tRange.LatFrcOpp_All   = [LA_timeline(5) LA_timeline(end)];
    mdata.(Instance).tRange.LatFrcOut       = [LA_timeline(6) LA_timeline(7)];
    mdata.(Instance).tRange.LongFrcBrk_All   = [LO_timeline(1) LO_timeline(4)];
    mdata.(Instance).tRange.LongFrcBrk      = [LO_timeline(2) LO_timeline(3)];
    mdata.(Instance).tRange.LongFrcAcc      = [LO_timeline(6) LO_timeline(7)];
    mdata.(Instance).tRange.LongFrcLBrk_All = [LOL_timeline(1) LOL_timeline(4)];
    ...
    mdata.(Instance).tRange.LatFrcOPara_All = [LAO_timeline(1) LAO_timeline(end)];
    mdata.(Instance).tRange.LatFrcOIn       = [LAO_timeline(2) LAO_timeline(3)];
    mdata.(Instance).tRange.LatFrcOOut      = [LAO_timeline(6) LAO_timeline(7)];
    mdata.(Instance).tRange.LatFrcOLeft_All = [LAOL_timeline(1) LAOL_timeline(end)];
    mdata.(Instance).tRange.LatFrcOLIn      = [LAOL_timeline(2) LAOL_timeline(3)];
    mdata.(Instance).tRange.LatFrcOLOut     = [LAOL_timeline(6) LAOL_timeline(7)];
    mdata.(Instance).tRange.LongFrcWCBrk_All= [LOW_timeline(1)  LOW_timeline(end)];
    mdata.(Instance).tRange.LongFrcWCBrk    = [LOW_timeline(2)  LOW_timeline(3)];
    mdata.(Instance).tRange.LongFrcWCLBrk_All= [LOWL_timeline(1) LOWL_timeline(end)];
