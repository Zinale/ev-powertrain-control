function maneuver_data = sm_car_maneuverdata_crg_pikes_peak
...
for i=1:length(Instance_List)
    ...
    if(startsWith(Instance,'Down_'))
        mdata.(Instance).Trajectory_LoadFile.Value    = 'CRG_Pikes_Peak_trajectory_down.mat';
    else
        mdata.(Instance).Trajectory_LoadFile.Value    = 'CRG_Pikes_Peak_trajectory_default.mat';
    end
